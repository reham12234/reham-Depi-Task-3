...
select top 10 order_id ,order_date from sales.orders order by order_date  --no data--
 ...
 select * from production.products where list_price between 500 and 1000     --no data--
  ...
  select * from production.products where category_id in(1,2,3)     --no data--
   ...
   select count(*) as total_num_of_products from production.products 

   select avg(list_price) as avg_of_price ,MIN(list_price) as min_price ,MAX(list_price) as max_price
   from production.products


   select category_id ,COUNT(product_id) as total_product from production.products
   group by category_id

     select store_id ,COUNT(order_id) as total_orders from sales.orders
   group by store_id   --no data--


   select top 10 upper(first_name) +' '+lower(last_name ) as full_name from sales.customers

   select top 10 product_name ,len(product_name) as lenght_of_product_name
   from production.products

   select customer_id ,left(phone,3) as area_code from sales.customers
   where customer_id between 1 and 15

   select order_id ,getdate() as current_datee ,Year(order_date) as yearr ,month(order_date) as monthh from sales.orders
   where order_id between 1 and 10   --no data--

   select top 10 product_name ,category_name from production.products p ,production.categories c
   where p.category_id=c.category_id

   select top 10 first_name+' '+last_name as customer_name ,order_date from sales.customers s ,sales.orders o
   where s.customer_id=o.customer_id    --no data--

   select product_name , coalesce(brand_name ,'No Brand') as brand_name from
   production.products p left outer join production.brands b on p.brand_id=b.brand_id

   select product_name ,list_price as price 
   from production.products
   where list_price >(select avg(list_price) from production.products)

   select customer_id ,first_name+' '+last_name as customer_name from sales.customers 
   where customer_id in(select distinct customer_id from sales.orders) --no data--

   select c
   (select count(*) from sales.orders o where o.customer_id=s.customer_id ) as total_orders from sales.customers s 
   ...
   state from sales.customers

    select * from customer_info where state='CA'    --no data--

    select * from sales.customers 

    select product_name ,list_price as price from production.products
    where list_price between 50 and 200 order by list_price asc

    select state ,count(customer_id) as customer_count from sales.customers
    group by state
    order by customer_count desc    

    select category_name  ,max(list_price) from production.products p ,production.categories c 
    where c.category_id=p.category_id group by c.category_name

    SELECT s.store_name,s.city,COUNT(o.order_id) AS 
    order_countFROM sales.stores sLEFT JOIN sales.orders o 
    ON s.store_id = o.store_idGROUP BY s.store_name, s.city;    --no data--
